...
router.get('/guilds/:guildId/members', authenticateToken, rbacMiddleware('user'), async (req, res) => {
  try {
    const { guildId } = req.params;
    const { limit = 100 } = req.query;
    const botUrl = process.env.BOT_API_URL || 'http://localhost:3002';
    
    const response = await fetch(`${botUrl}/api/guild/${guildId}/members?limit=${limit}`, {
